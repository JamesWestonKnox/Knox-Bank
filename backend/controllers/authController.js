...
export const login = async (req, res) => {
  await body("accountNumber")
    ...
    .run(req); //Validating account number
  ...
  try {
    ...
    if (!customer)
      return res
        .status(401)
        .json({ error: "Account number incorrect or user does not exist" });
    ...
    //Success response
    res.json({
      user: { id: customer._id, fullName: customer.fullName, accountNumber },
    });
  } catch (err) {
...
//Method allowing the employee to login
export const loginEmployee = async (req, res) => {
  await body("employeeNumber")
    .matches(/^EMP\d{4}$/)
    .withMessage("Employee number must start with EMP with 4 digits afterwards")
    .run(req); //Validating employee number
  ...
  //Trying to find the corrosponding employee to the employee number
  try {
    const employee = await Employee.findOne({ employeeNumber });

    //If employee number is not found then error message
    if (!employee)
      return res
        .status(401)
        .json({
          error: "Invalid employee number format or employee does not exist",
        });

    //If there is a employee with that employee number, then checking if passwords match
    ...
    //Creating a jwt token to keep the user logged in
    const token = jwt.sign(
      { id: employee._id, fullName: employee.fullName, employeeNumber },
      process.env.JWT_SECRET,
      { expiresIn: "15m" } //jwt token only lasts for 15 minutes
    );
